      ...
      const user_key = await validateToken(req, res);
        ...
        if(user_key) {
            const chats = await db.query(
                ...
                WHERE chats.user1 = $1`, [user_key.user.rows[0].id]
            );
...
exports.initiate_chat = async (req, res) => {
    try {
        const user_key = await validateToken(req, res);
        ...
        if(user_key) {
            ...
            const chat = await db.query(`SELECT * FROM chats WHERE user1 = $1 AND user2 = $2`, 
                [user_key.user.rows[0].id, user.rows[0].id]
            );

            if(chat.rows.length === 0){
                const new_chat = await db.query(`INSERT INTO chats (user1, user2, last_message_sent) VALUES ($1, $2, $3)`, 
                    [user_key.user.rows[0].id, user.rows[0].id, null]
                );
    ...
    try {
        ...
        if(user_key) {
            ...
            let partner_chat = await db.query(`SELECT * FROM chats WHERE user1 = $1 AND user2 = $2`, 
                [receiving_user.rows[0].id, user_key.user.rows[0].id]
            );
            ...
            await db.query(`UPDATE chats SET last_message_sent = $1 WHERE user1 = $2 AND user2 = $3`, 
                [message.rows[0].id, user_key.user.rows[0].id, receiving_user.rows[0].id]
            );

            if(partner_chat.rows.length === 0) {
                partner_chat = await db.query(`INSERT INTO chats (user1, user2, last_message_sent) VALUES ($1, $2, $3)
                    RETURNING *`, 
                    [receiving_user.rows[0].id, user_key.user.rows[0].id, message.rows[0].id]
                );
        ...
        if(user_key) {
            ...
            if(req.body.accept === 'false') {
                await db.query(`DELETE FROM chats WHERE user1 = $1 AND user2 = $2`, 
                    [user_key.user.rows[0].id, request.rows[0].requesting_user]
                );

                await db.query(`DELETE FROM chats WHERE user1 = $1 AND user2 = $2`, 
                    [request.rows[0].requesting_user, user_key.user.rows[0].id]
                );
